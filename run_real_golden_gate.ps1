param(
    [string]$Manifest = "real_golden_manifest.json",
...
foreach ($title in $manifestObj.titles) {
    ...
    "[REAL-GOLDEN] title=$titleId format=$format pass=$overall note=$note"
}
...
"[REAL-GOLDEN] summary pass=$passCount/$totalCount report=$reportPath"
