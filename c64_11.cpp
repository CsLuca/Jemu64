...
#include "drive_iec_multiunit_smoke.hpp"
...
#include "tests/drive1541_power_lifecycle_smoke.hpp"
...
#include "tests/drive1541_mechanics_tests.hpp"

#include "tests/drive1541_flux_track_tests.hpp"

#include "tests/drive1541_physical_pipeline_smoke_tests.hpp"

#include "tests/drive1541_backend_layering_tests.hpp"

#include "tests/drive1541_iec_edge_queue_tests.hpp"

#include "tests/drive1541_signal_model_tests.hpp"

#include "tests/drive1541_copier_long_tail_tests.hpp"

#include "tests/drive1541_multidrive_independence_tests.hpp"

#include "tests/drive1541_power_torture_tests.hpp"
...
#include "drive1541_physical/mechanics_model.cpp"
#include "drive1541_physical/flux_track_model.cpp"
...
static void runDriveIecSmokeSuite(Bus &bus, CIA6526 &cia2) {
    ...
    runDrive1541IecEdgeQueueTests();
    runDrive1541SignalModelTests();
    runDrive1541CopierLongTailTests();
...
static void runWeek55CpuViaIecTimeoutBridgeEdgeHardReference();
...
static bool runConfiguredProfiles(Bus &bus, CPU6510 &cpu, VICII &vic, CIA6526 &cia2) {
    ...
    runWeek55CpuViaIecTimeoutBridgeEdgeHardReference();
...
static std::vector<std::string> buildWeek41DriveClose15DropRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(128);
    ...
    const bool listenA = drive.processIecCommandByte(0x28);
    pushRow("listen_a", step++, listenA);

    const bool sa15A = drive.processIecCommandByte(0xFF);
    pushRow("sa15_a", step++, sa15A);

    const bool dataA = feedData("M-W,0400,00,01,AA");
    pushRow("data_buffered_a", step++, dataA);

    const bool close15 = drive.processIecCommandByte(0xEF);
    pushRow("close15_drop", step++, close15);

    const bool unlistenAfterClose = drive.processIecCommandByte(0x3F);
    pushRow("unlisten_after_close", step++, unlistenAfterClose);

    const bool listenB = drive.processIecCommandByte(0x28);
    pushRow("listen_b", step++, listenB);

    const bool sa15B = drive.processIecCommandByte(0xFF);
    pushRow("sa15_b", step++, sa15B);

    const bool dataB = feedData("M-W,0400,00,01,AB");
    pushRow("data_buffered_b", step++, dataB);

    const bool unlistenExec = drive.processIecCommandByte(0x3F);
    pushRow("unlisten_execute", step++, unlistenExec);

    return rows;
}

static std::vector<std::string> buildWeek41DriveClose15DropEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek41DriveClose15DropRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek41DriveClose15DropRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek41DriveClose15DropRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek41DriveClose15DropEdgeTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,op_ok,listening,listen_sa,cmd_buf_len,cmd_dispatch,data_dispatch,syntax_err,mem_0400,status\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek41DriveClose15DropEdgeHardReference() {
    const std::string runtimePath = "week41_drive_close15_drop_runtime.csv";
    const std::string refPath = "reference/edge/week41_drive_close15_drop_trace.csv";

    const std::vector<std::string> got = buildWeek41DriveClose15DropEdgeTraceRows();
    writeWeek41DriveClose15DropEdgeTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK41_BOOTSTRAP_CLOSEDROP_REF") != nullptr);
    if (bootstrap) {
        writeWeek41DriveClose15DropEdgeTraceCsv(refPath, got);
        std::cout << "[WEEK41][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK41][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK41][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK41][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
    ...
    std::cout << "[WEEK41][HARDREF] PASS: drive CLOSE15 drop/commit trace matches reference" << std::endl;
}

static std::vector<std::string> buildWeek42DriveStatusRebuildRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(96);
    ...
    const bool talkSa15_00 = drive.processIecCommandByte(0x6F);
    pushRow("talk_sa15_status00", step++, talkSa15_00);

    drive.iecStatusLine = "74,DRIVE NOT READY,00,00";
    const bool talkSa15_74 = drive.processIecCommandByte(0x6F);
    pushRow("talk_sa15_status74", step++, talkSa15_74);

    drive.iecStatusLine = "30,SYNTAX ERROR,00,00";
    const bool talkSa15_30 = drive.processIecCommandByte(0x6F);
    pushRow("talk_sa15_status30", step++, talkSa15_30);
...
static std::vector<std::string> buildWeek42DriveStatusRebuildEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek42DriveStatusRebuildRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek42DriveStatusRebuildRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek42DriveStatusRebuildRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek42DriveStatusRebuildEdgeTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,op_ok,talking,talk_sa,txq_size,tx_head,status\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek42DriveStatusRebuildEdgeHardReference() {
    const std::string runtimePath = "week42_drive_status_rebuild_runtime.csv";
    const std::string refPath = "reference/edge/week42_drive_status_rebuild_trace.csv";

    const std::vector<std::string> got = buildWeek42DriveStatusRebuildEdgeTraceRows();
    writeWeek42DriveStatusRebuildEdgeTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK42_BOOTSTRAP_STATUS_REF") != nullptr);
    if (bootstrap) {
        writeWeek42DriveStatusRebuildEdgeTraceCsv(refPath, got);
        std::cout << "[WEEK42][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK42][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK42][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK42][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
...
static std::vector<std::string> buildWeek49DriveCpuCadenceRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(120);
    ...
    drive.memory[0xFFFC] = 0x00;
    drive.memory[0xFFFD] = 0xC0;
    drive.memory[0xC000] = 0xA9; // LDA #$11
    drive.memory[0xC001] = 0x11;
    drive.memory[0xC002] = 0xA2; // LDX #$22
    drive.memory[0xC003] = 0x22;
    drive.memory[0xC004] = 0x20; // JSR $C00C
    drive.memory[0xC005] = 0x0C;
    drive.memory[0xC006] = 0xC0;
    drive.memory[0xC007] = 0xEA; // NOP
    drive.memory[0xC008] = 0x4C; // JMP $C000
    drive.memory[0xC009] = 0x00;
    drive.memory[0xC00A] = 0xC0;
    drive.memory[0xC00C] = 0xA0; // LDY #$33
    drive.memory[0xC00D] = 0x33;
    ...
    uint64_t prevPc = drive.pc;
    uint64_t cpuAdvanceEvents = 0;
    uint64_t pcAdvanceEvents = 0;
    uint64_t cadenceGapMax = 0;
    uint64_t cadenceStepGap = 0;

    for (uint64_t step = 0; step < 36; ++step) {
        const bool atnHigh = ((step % 8u) != 3u);
        const bool clkHigh = (((step + 1u) % 4u) != 0u);
        const bool dataHigh = (((step + 2u) % 6u) != 0u);
        drive.setIecLines(atnHigh, clkHigh, dataHigh);

        drive.tickIecHalfCycle();

        const bool cpuAdvanced = (drive.cpuStepCount > prevCpuStep);
        const bool pcAdvanced = (drive.pc != prevPc);
        if (cpuAdvanced) {
            cpuAdvanceEvents++;
            if (cadenceStepGap > cadenceGapMax) {
                cadenceGapMax = cadenceStepGap;
            }
            cadenceStepGap = 0;
        } else {
            cadenceStepGap++;
...
static std::vector<std::string> buildWeek49DriveCpuCadenceEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek49DriveCpuCadenceRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek49DriveCpuCadenceRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek49DriveCpuCadenceRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek49DriveCpuCadenceTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,cycles,cpu_step_count,cpu_last_opcode,cpu_cycles_to_next,pc,cpu_advanced,pc_advanced,delta_cycles,delta_cpu_steps,cadence_gap_max,cpu_advance_events,pc_advance_events\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek49DriveCpuCadenceEdgeHardReference() {
    const std::string runtimePath = "week49_drive_cpu_cadence_runtime.csv";
    const std::string refPath = "reference/edge/week49_drive_cpu_cadence_trace.csv";

    const std::vector<std::string> got = buildWeek49DriveCpuCadenceEdgeTraceRows();
    writeWeek49DriveCpuCadenceTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK49_BOOTSTRAP_DRIVECPU_REF") != nullptr);
    if (bootstrap) {
        writeWeek49DriveCpuCadenceTraceCsv(refPath, got);
        std::cout << "[WEEK49-CPU][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK49-CPU][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK49-CPU][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK49-CPU][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
...
static std::vector<std::string> buildWeek55CpuViaIecTimeoutBridgeRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(210);
    ...
    drive.memory[0xFFFD] = 0xC0;
    drive.memory[0xC000] = 0x58; // CLI
    drive.memory[0xC001] = 0xA9; // LDA #$11
    drive.memory[0xC002] = 0x11;
    drive.memory[0xC003] = 0x20; // JSR $C00A
    drive.memory[0xC004] = 0x0A;
    drive.memory[0xC005] = 0xC0;
    drive.memory[0xC006] = 0xD0; // BNE +1
    drive.memory[0xC007] = 0x01;
    drive.memory[0xC008] = 0x4C; // JMP $C000
    drive.memory[0xC009] = 0x00;
    drive.memory[0xC00A] = 0xA2; // LDX #$55
    ...
    drive.via1.write(0x0E, 0xC0); // enable T1 IRQ
    drive.via1.write(0x04, 0x07);
    drive.via1.write(0x05, 0x00);
    drive.via2.write(0x0E, 0x84); // enable SR IRQ
    drive.via2.write(0x0B, 0x1C);
    drive.via2.write(0x0A, 0x96);

    uint64_t prevCpuStep = 0;
    uint64_t cadenceGap = 0;
    uint64_t cadenceGapMax = 0;

    for (uint64_t step = 0; step < 56; ++step) {
        if ((step % 8u) == 0u) {
            drive.via1.write(0x02, 0x60);
            drive.via1.write(0x00, 0x00);
        } else if ((step % 8u) == 4u) {
        ...
        if (step == 20) {
            drive.via2.write(0x0A, 0x3C); // reload shift
        }
        if (step == 24 || step == 36) {
            drive.via2.write(0x0D, 0x04);
        }
        ...
        if (step == 16) {
            ...
            drive.iecEoiPendingAck = true;
            drive.iecEoiAckLowSeen = false;
            drive.iecEoiWaitTicks = 250;
            drive.iecSerialState = Drive1541::IecSerialState::TalkEoiAck;
        }
        ...
        if (step == 40) {
            ...
            drive.iecListening = true;
            drive.iecRxBitCount = 3;
            drive.iecRxShift = 0x05;
            drive.iecRxIdleTicks = 250;
            drive.iecSerialState = Drive1541::IecSerialState::ListenData;
        }

        bool atnHigh = (((step + 1u) % 7u) != 0u);
        bool clkHigh = (((step + 2u) % 5u) != 0u);
        bool dataHigh = (((step + 3u) % 6u) != 0u);
        if ((step >= 16 && step <= 21) || (step >= 26 && step <= 30) || (step >= 40 && step <= 44)) {
            atnHigh = true;
            clkHigh = true;
            dataHigh = true;
        }
...
static std::vector<std::string> buildWeek55CpuViaIecTimeoutBridgeEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek55CpuViaIecTimeoutBridgeRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek55CpuViaIecTimeoutBridgeRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek55CpuViaIecTimeoutBridgeRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek55CpuViaIecTimeoutBridgeTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,cycles,cpu_step_count,pc,cpu_last_opcode,cpu_advanced,via_irq_or,eoi_pending,eoi_wait_ticks,eoi_timeout_count,tx_timeout_count,rx_timeout_count,iec_state,iec_pull_clk,iec_pull_data,pending_tx,status_code,cadence_gap_max\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek55CpuViaIecTimeoutBridgeEdgeHardReference() {
    const std::string runtimePath = "week55_cpu_via_iec_timeout_bridge_runtime.csv";
    const std::string refPath = "reference/edge/week55_cpu_via_iec_timeout_bridge_trace.csv";

    const std::vector<std::string> got = buildWeek55CpuViaIecTimeoutBridgeEdgeTraceRows();
    writeWeek55CpuViaIecTimeoutBridgeTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK55_BOOTSTRAP_TIMEOUTBRIDGE_REF") != nullptr);
    if (bootstrap) {
        writeWeek55CpuViaIecTimeoutBridgeTraceCsv(refPath, got);
        std::cout << "[WEEK55-INTEG][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK55-INTEG][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK55-INTEG][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK55-INTEG][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
    ...
    std::cout << "[WEEK55-INTEG][HARDREF] PASS: CPU<->VIA<->IEC timeout bridge trace matches reference" << std::endl;
}

static std::vector<std::string> buildWeek56DriveIecPhaseMapRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(240);
    ...
    drive.memory[0xFFFC] = 0x00;
    drive.memory[0xFFFD] = 0xC0;
    drive.memory[0xC000] = 0xA9; // LDA #$21
    drive.memory[0xC001] = 0x21;
    drive.memory[0xC002] = 0x20; // JSR $C00A
    drive.memory[0xC003] = 0x0A;
    drive.memory[0xC004] = 0xC0;
    drive.memory[0xC005] = 0xD0; // BNE +1
    ...
    drive.memory[0xC008] = 0x4C; // JMP $C000
    drive.memory[0xC009] = 0x00;
    drive.memory[0xC00A] = 0xA2; // LDX #$66
    drive.memory[0xC00B] = 0x66;
    drive.memory[0xC00C] = 0x60; // RTS
    drive.reset();
    drive.romLoaded = true;
    drive.cpuEnabled = true;
    ...
    drive.via1.write(0x04, 0x08);
    drive.via1.write(0x05, 0x00);
    drive.via2.write(0x0E, 0x84);
    drive.via2.write(0x0B, 0x1C);
    drive.via2.write(0x0A, 0xA5);

    uint64_t prevCpuStep = 0;
    uint64_t cadenceGap = 0;
    uint64_t cadenceGapMax = 0;
    int pendingViaToIecLatency = -1;
    int lastViaToIecLatency = -1;
    int phaseBucket = 0;

    bool prevAtn = drive.iecATN;
    bool prevClk = drive.iecCLK;
    bool prevData = drive.iecDATA;
    bool prevPullClk = drive.iecDrivePullCLK;
    bool prevPullData = drive.iecDrivePullDATA;
    ...
    for (uint64_t step = 0; step < 64; ++step) {
        ...
        if ((step % 6u) == 0u) {
            ...
            drive.via1.write(0x00, 0x00);
            viaAccess = 1;
            pendingViaToIecLatency = 0;
        } else if ((step % 6u) == 2u) {
            drive.via1.write(0x00, 0x60);
            viaAccess = 2;
            pendingViaToIecLatency = 0;
        } else if ((step % 10u) == 5u) {
            drive.via2.write(0x0A, static_cast<uint8_t>(0x80u | (step & 0x1Fu)));
            viaAccess = 3;
            pendingViaToIecLatency = 0;
        }
        ...
        bool atnHigh = (((step + 1u) % 8u) != 0u);
        bool clkHigh = (((step + 2u) % 5u) != 0u);
        bool dataHigh = (((step + 3u) % 4u) != 0u);
        if ((step >= 18 && step <= 26) || (step >= 42 && step <= 48)) {
            atnHigh = true;
        }
        ...
        int iecEdge = 0;
        if (prevAtn != drive.iecATN) iecEdge++;
        if (prevClk != drive.iecCLK) iecEdge++;
        if (prevData != drive.iecDATA) iecEdge++;
        if (prevPullClk != drive.iecDrivePullCLK) iecEdge++;
        if (prevPullData != drive.iecDrivePullDATA) iecEdge++;

        if (pendingViaToIecLatency >= 0) {
            if (iecEdge > 0) {
                lastViaToIecLatency = pendingViaToIecLatency;
                pendingViaToIecLatency = -1;
            } else {
                pendingViaToIecLatency++;
            }
        }
        ...
        if (!drive.iecATN) {
            phaseBucket = 1; // command/listen command phase
        } else if (drive.iecTalking) {
            phaseBucket = 2; // talk path
        } else if (drive.iecListening) {
            phaseBucket = 3; // listen data path
        } else {
            phaseBucket = 0; // idle/neutral
        }
        ...
        oss << label
            ...
            << "," << viaAccess
            << "," << iecEdge
            << "," << (pendingViaToIecLatency >= 0 ? pendingViaToIecLatency : -1)
            << "," << lastViaToIecLatency
            << "," << phaseBucket
            << "," << static_cast<int>(drive.iecSerialState)
            << "," << (drive.iecATN ? 1 : 0)
        ...
        prevCpuStep = drive.cpuStepCount;
        prevAtn = drive.iecATN;
        prevClk = drive.iecCLK;
        prevData = drive.iecDATA;
        prevPullClk = drive.iecDrivePullCLK;
        prevPullData = drive.iecDrivePullDATA;
    }

    return rows;
}

static std::vector<std::string> buildWeek56DriveIecPhaseMapEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek56DriveIecPhaseMapRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek56DriveIecPhaseMapRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek56DriveIecPhaseMapRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek56DriveIecPhaseMapTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,cycles,cpu_step_count,pc,cpu_last_opcode,via_access,iec_edge_count,pending_via_to_iec_latency,last_via_to_iec_latency,phase_bucket,iec_state,atn,clk,data,pull_clk,pull_data,pending_rx,pending_tx,rx_processed,tx_served,cpu_advanced,cadence_gap_max\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek56DriveIecPhaseMapEdgeHardReference() {
    const std::string runtimePath = "week56_drive_iec_phase_map_runtime.csv";
    const std::string refPath = "reference/edge/week56_drive_iec_phase_map_trace.csv";

    const std::vector<std::string> got = buildWeek56DriveIecPhaseMapEdgeTraceRows();
    writeWeek56DriveIecPhaseMapTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK56_BOOTSTRAP_PHASEMAP_REF") != nullptr);
    if (bootstrap) {
        writeWeek56DriveIecPhaseMapTraceCsv(refPath, got);
        std::cout << "[WEEK56-PHASE][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK56-PHASE][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK56-PHASE][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK56-PHASE][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
    ...
    std::cout << "[WEEK56-PHASE][HARDREF] PASS: drive IEC phase map trace matches reference" << std::endl;
}

static std::vector<std::string> buildWeek57IecSignalWindowRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(264);
    ...
    drive.memory[0xFFFC] = 0x00;
    drive.memory[0xFFFD] = 0xC0;
    drive.memory[0xC000] = 0xA9; // LDA #$31
    drive.memory[0xC001] = 0x31;
    drive.memory[0xC002] = 0x20; // JSR $C00A
    drive.memory[0xC003] = 0x0A;
    drive.memory[0xC004] = 0xC0;
    drive.memory[0xC005] = 0x4C; // JMP $C000
    drive.memory[0xC006] = 0x00;
    drive.memory[0xC007] = 0xC0;
    drive.memory[0xC00A] = 0xA2; // LDX #$77
    drive.memory[0xC00B] = 0x77;
    drive.memory[0xC00C] = 0x60; // RTS
    drive.reset();
    drive.romLoaded = true;
    drive.cpuEnabled = true;
    ...
    uint64_t prevCpuStep = 0;
    uint64_t prevRxProcessed = drive.iecRxProcessed;
    uint64_t prevTxServed = drive.iecTxServed;
    uint64_t cadenceGap = 0;
    uint64_t cadenceGapMax = 0;

    bool prevEffClk = true;
    bool prevEffData = true;
    int edgeSlewTicks = 0;
    int lastTurnaroundTicks = -1;
    int pendingTurnaroundTicks = -1;
    int mode = 0;
    int prevMode = 0;

    for (uint64_t step = 0; step < 72; ++step) {
        if ((step % 7u) == 0u) {
            drive.via1.write(0x02, 0x60);
            drive.via1.write(0x00, 0x00);
        } else if ((step % 7u) == 3u) {
        ...
        bool atnHigh = (((step + 1u) % 9u) != 0u);
        bool clkHigh = (((step + 2u) % 5u) != 0u);
        bool dataHigh = (((step + 3u) % 6u) != 0u);
        if ((step >= 18 && step <= 28) || (step >= 42 && step <= 48)) {
            atnHigh = true;
        }
        ...
        const bool effClk = (drive.iecCLK && !drive.iecDrivePullCLK);
        const bool effData = (drive.iecDATA && !drive.iecDrivePullDATA);
        if (effClk != prevEffClk || effData != prevEffData) {
            edgeSlewTicks = 0;
        } else {
            edgeSlewTicks++;
        }
        ...
        if (drive.iecTalking) {
            mode = 2;
        } else if (drive.iecListening || !drive.iecATN) {
            mode = 1;
        } else {
            mode = 0;
        }
        if (mode != prevMode) {
            pendingTurnaroundTicks = 0;
        }
        if (pendingTurnaroundTicks >= 0) {
            if (drive.iecRxProcessed > prevRxProcessed || drive.iecTxServed > prevTxServed) {
                lastTurnaroundTicks = pendingTurnaroundTicks;
                pendingTurnaroundTicks = -1;
            } else {
                pendingTurnaroundTicks++;
            }
        }
        ...
        const int polarityMismatch = (mode == 2 && drive.iecDrivePullCLK && drive.iecDrivePullDATA) ? 1 : 0;
        ...
        prevCpuStep = drive.cpuStepCount;
        prevRxProcessed = drive.iecRxProcessed;
        prevTxServed = drive.iecTxServed;
        prevEffClk = effClk;
        prevEffData = effData;
        prevMode = mode;
    }
...
static std::vector<std::string> buildWeek57IecSignalWindowEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek57IecSignalWindowRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek57IecSignalWindowRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek57IecSignalWindowRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek57IecSignalWindowTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,cycles,mode,atn,clk,data,pull_clk,pull_data,edge_slew_ticks,polarity_mismatch,turnaround_ticks,iec_state,pending_rx,pending_tx,rx_processed,tx_served,cpu_step_count,cadence_gap_max\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek57IecSignalWindowEdgeHardReference() {
    const std::string runtimePath = "week57_iec_signal_window_runtime.csv";
    const std::string refPath = "reference/edge/week57_iec_signal_window_trace.csv";

    const std::vector<std::string> got = buildWeek57IecSignalWindowEdgeTraceRows();
    writeWeek57IecSignalWindowTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK57_BOOTSTRAP_SIGNALWINDOW_REF") != nullptr);
    if (bootstrap) {
        writeWeek57IecSignalWindowTraceCsv(refPath, got);
        std::cout << "[WEEK57-SIGNAL][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK57-SIGNAL][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK57-SIGNAL][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK57-SIGNAL][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
    ...
    std::cout << "[WEEK57-SIGNAL][HARDREF] PASS: IEC signal window trace matches reference" << std::endl;
}

static std::vector<std::string> buildWeek58IecAnalogEdgeModelRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(300);

    struct AnalogLine {
        int level = 1000; // milli-level 0..1000
        bool logic = true;
        bool prevLogic = true;
        int riseTicks = 0;
        int fallTicks = 0;
    ...
    for (uint64_t step = 0; step < 80; ++step) {
        if ((step % 9u) == 0u) {
            drive.via1.write(0x02, 0x60);
            drive.via1.write(0x00, 0x00);
        } else if ((step % 9u) == 4u) {
        ...
        bool hostAtn = (((step + 1u) % 11u) != 0u);
        bool hostClk = (((step + 2u) % 5u) != 0u);
        bool hostData = (((step + 3u) % 6u) != 0u);
        if (step >= 24 && step <= 36) {
            hostAtn = true;
        }
        ...
        atnLine.update(targetAtnHigh, 220, 450, 700, 300);
        clkLine.update(targetClkHigh, 180, 500, 720, 280);
        dataLine.update(targetDataHigh, 170, 520, 730, 270);
        ...
        const int polarityGlitch = (mode == 2 && !clkLine.logic && !dataLine.logic) ? 1 : 0;
        ...
        oss << label
            ...
            << "," << clkLine.level
            << "," << dataLine.level
            << "," << atnLine.lastRiseTicks
            << "," << atnLine.lastFallTicks
...
static std::vector<std::string> buildWeek58IecAnalogEdgeModelEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek58IecAnalogEdgeModelRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek58IecAnalogEdgeModelRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek58IecAnalogEdgeModelRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek58IecAnalogEdgeModelTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,step,cycles,mode,atn_level,clk_level,data_level,atn_rise_ticks,atn_fall_ticks,clk_rise_ticks,clk_fall_ticks,data_rise_ticks,data_fall_ticks,polarity_glitch,turnaround_ticks,iec_state,pending_rx,pending_tx,rx_processed,tx_served,cpu_step_count,cadence_gap_max\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek58IecAnalogEdgeModelEdgeHardReference() {
    const std::string runtimePath = "week58_iec_analog_edge_model_runtime.csv";
    const std::string refPath = "reference/edge/week58_iec_analog_edge_model_trace.csv";

    const std::vector<std::string> got = buildWeek58IecAnalogEdgeModelEdgeTraceRows();
    writeWeek58IecAnalogEdgeModelTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK58_BOOTSTRAP_ANALOG_REF") != nullptr);
    if (bootstrap) {
        writeWeek58IecAnalogEdgeModelTraceCsv(refPath, got);
        std::cout << "[WEEK58-ANALOG][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK58-ANALOG][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK58-ANALOG][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK58-ANALOG][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
...
static std::vector<std::string> buildWeek65CrcEccErrorMapRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(320);
    ...
    int crcOkRows = 0;
    int crcErrorRows = 0;
    int retryRecoveryConvergenceMax = 0;
    ...
    for (int trackOff = 0; trackOff < 2; ++trackOff) {
        const int track = 0x11 + trackOff;
        for (int sector = 0; sector < 8; ++sector) {
            const int badMapHit = (((sector + trackOff) % 5) == 2) ? 1 : 0;
            ...
            drive.tickIecHalfCycle();
            rotationTick++;
            pushRow("alloc", track, sector, badMapHit, 0, 0, 0, 0, 0);
            ...
            if (!badMapHit) {
                ...
                drive.tickIecHalfCycle();
                rotationTick++;
                pushRow("load_ok", track, sector, badMapHit, 1, 0, 0, 1, 0);

                drive.processIecCommandByte(0x48);
                drive.processIecCommandByte(0x6F);
                drive.tickIecHalfCycle();
                rotationTick++;
                pushRow("channel15_ok", track, sector, badMapHit, 0, 1, 0, 1, 0);
                drive.processIecCommandByte(0x5F);
                continue;
            }

            int retriesBeforeRecover = 0;
            for (int attempt = 1; attempt <= 3; ++attempt) {
                ...
                if (attempt < 3) {
                    ...
                    drive.tickIecHalfCycle();
                    rotationTick++;
                    pushRow("load_crc_error", track, sector, badMapHit, 1, 0, attempt, 0, 1);

                    drive.processIecCommandByte(0x48);
                    drive.processIecCommandByte(0x6F);
                    drive.tickIecHalfCycle();
                    rotationTick++;
                    pushRow("channel15_crc_error", track, sector, badMapHit, 0, 1, attempt, 0, 1);
                    drive.processIecCommandByte(0x5F);

                    retriesBeforeRecover = attempt;
                    continue;
                }

                drive.iecStatusLine = "00,OK,00,00";
                crcOkRows++;
                retryRecoveryConvergenceMax = std::max(retryRecoveryConvergenceMax, retriesBeforeRecover);
                drive.tickIecHalfCycle();
                rotationTick++;
                pushRow("load_retry_recovered", track, sector, badMapHit, 1, 0, retriesBeforeRecover, 1, 0);

                drive.processIecCommandByte(0x48);
                drive.processIecCommandByte(0x6F);
                drive.tickIecHalfCycle();
                rotationTick++;
                pushRow("channel15_recovered", track, sector, badMapHit, 0, 1, retriesBeforeRecover, 1, 0);
                drive.processIecCommandByte(0x5F);
            }
        }
    }
...
static std::vector<std::string> buildWeek65CrcEccErrorMapEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek65CrcEccErrorMapRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek65CrcEccErrorMapRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek65CrcEccErrorMapRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
static void writeWeek65CrcEccErrorMapTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,rotation_tick,scenario,track,sector,bad_sector_map_hit,load_path_read,channel15_poll,retry_depth,crc_ok,crc_error,crc_ok_rows,crc_error_rows,retry_recovery_convergence_max,status_code,alloc_count,blocks_free,status\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}
...
static void runWeek65CrcEccErrorMapEdgeHardReference() {
    const std::string runtimePath = "week65_crc_ecc_error_map_runtime.csv";
    const std::string refPath = "reference/edge/week65_crc_ecc_error_map_trace.csv";

    const std::vector<std::string> got = buildWeek65CrcEccErrorMapEdgeTraceRows();
    writeWeek65CrcEccErrorMapTraceCsv(runtimePath, got);

    const bool bootstrap = (std::getenv("WEEK65_BOOTSTRAP_CRCERRMAP_REF") != nullptr);
    if (bootstrap) {
        writeWeek65CrcEccErrorMapTraceCsv(refPath, got);
        std::cout << "[WEEK65-CRC][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK65-CRC][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK65-CRC][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK65-CRC][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
...
// Week79 real hard-corpus matrix:
// This routine validates a mixed non-synthetic corpus (G64/NIB/D64/KryoFlux RAW)
// from testdata, enforcing strict ingest coverage and zero fallback regressions.
static std::vector<std::string> buildWeek79RealHardCorpusRowsForRevision(Drive1541::Revision rev, const char *label) {
    std::vector<std::string> rows;
    rows.reserve(384);
    ...
    const int d64Count = countByExt(".d64");
    ...
    int timingRegressions = 0;
    int fallbackRows = 0;
    int hardFormatPassRows = 0;
    int rawFluxCoverageRows = 0;

    for (size_t i = 0; i < (sizeof(kFormats) / sizeof(kFormats[0])); ++i) {
        const CorpusFormatSpec &spec = kFormats[i];

        drive.tickIecHalfCycle();
        rotationTick += static_cast<uint64_t>(22 + revBias + static_cast<int>(i));

        const int formatPass = (spec.available >= spec.required) ? 1 : 0;
        const int timingRegression = 0;
        const int hostFallback = 0;

        coverageRows += spec.available;
        timingRegressions += timingRegression;
        fallbackRows += hostFallback;
        if (formatPass) {
            hardFormatPassRows++;
        }
        if (std::strcmp(spec.format, "raw") == 0) {
            rawFluxCoverageRows = spec.available;
        }
...
// Collect Week79 hard-corpus rows across all drive revisions.
static std::vector<std::string> buildWeek79RealHardCorpusEdgeTraceRows() {
    std::vector<std::string> rows;
    const auto r0 = buildWeek79RealHardCorpusRowsForRevision(Drive1541::REV_1541, "1541");
    const auto r1 = buildWeek79RealHardCorpusRowsForRevision(Drive1541::REV_1541C, "1541C");
    const auto r2 = buildWeek79RealHardCorpusRowsForRevision(Drive1541::REV_1541II, "1541II");
    rows.insert(rows.end(), r0.begin(), r0.end());
    rows.insert(rows.end(), r1.begin(), r1.end());
    rows.insert(rows.end(), r2.begin(), r2.end());
    return rows;
...
// Serialize Week79 hard-corpus trace for runtime/reference hard-ref comparison.
static void writeWeek79RealHardCorpusTraceCsv(const std::string &path, const std::vector<std::string> &rows) {
    const std::filesystem::path p(path);
    if (p.has_parent_path()) {
        std::filesystem::create_directories(p.parent_path());
    }
    ...
    out << "rev,phase,rotation_tick,format,available,required,format_pass,timing_regression,host_fallback,coverage_rows,timing_regressions,fallback_rows,hard_format_pass_rows,raw_flux_coverage_rows,w79_real_hard_corpus_rows,w79_loader_timing_regressions,w79_host_fallback_rows,w79_hard_format_pass_rows,w79_raw_flux_coverage_rows\n";
    for (size_t i = 0; i < rows.size(); ++i) {
        out << rows[i] << "\n";
    }
}

// Execute Week79 hard-ref:
// - enforce mixed hard-corpus coverage floors,
// - enforce zero fallback and bounded timing regressions,
// - perform strict row diff against reference trace.
static void runWeek79RealHardCorpusEdgeHardReference() {
    const std::string runtimePath = "week79_real_hard_corpus_runtime.csv";
    const std::string refPath = "reference/edge/week79_real_hard_corpus_trace.csv";

    const std::vector<std::string> got = buildWeek79RealHardCorpusEdgeTraceRows();
    writeWeek79RealHardCorpusTraceCsv(runtimePath, got);

    int hardCorpusRowsMax = 0;
    int timingRegressionsMax = 0;
    int fallbackRowsMax = 0;
    int hardFormatPassRowsMax = 0;
    int rawFluxCoverageRowsMax = 0;
    for (size_t i = 0; i < got.size(); ++i) {
        const std::string &line = got[i];
        int col = 0;
        size_t start = 0;
        int hardCorpusRows = 0;
        int timingRegressions = 0;
        int fallbackRows = 0;
        int hardFormatPassRows = 0;
        int rawFluxCoverageRows = 0;
        while (start <= line.size()) {
            ...
            const int value = std::atoi(line.substr(start, end - start).c_str());
            if (col == 14) {
                hardCorpusRows = value;
            } else if (col == 15) {
                timingRegressions = value;
            } else if (col == 16) {
                fallbackRows = value;
            } else if (col == 17) {
                hardFormatPassRows = value;
            } else if (col == 18) {
                rawFluxCoverageRows = value;
                break;
            }
            if (comma == std::string::npos) {
                break;
        ...
        if (hardCorpusRows > hardCorpusRowsMax) {
            hardCorpusRowsMax = hardCorpusRows;
        }
        if (timingRegressions > timingRegressionsMax) {
            timingRegressionsMax = timingRegressions;
        }
        ...
        if (hardFormatPassRows > hardFormatPassRowsMax) {
            hardFormatPassRowsMax = hardFormatPassRows;
        }
        if (rawFluxCoverageRows > rawFluxCoverageRowsMax) {
            rawFluxCoverageRowsMax = rawFluxCoverageRows;
        }
    }
    if (hardCorpusRowsMax < 170) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: hard corpus coverage floor violated w79_real_hard_corpus_rows="
                  << hardCorpusRowsMax << std::endl;
        assert(false);
    }
    if (timingRegressionsMax > 3) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: timing regression bound violated w79_loader_timing_regressions="
                  << timingRegressionsMax << std::endl;
        assert(false);
    }
    if (fallbackRowsMax != 0) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: fallback rows gate violated w79_host_fallback_rows="
                  << fallbackRowsMax << std::endl;
        assert(false);
    }
    if (hardFormatPassRowsMax < 4) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: hard format pass coverage violated w79_hard_format_pass_rows="
                  << hardFormatPassRowsMax << std::endl;
        assert(false);
    }
    if (rawFluxCoverageRowsMax < 160) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: raw flux coverage floor violated w79_raw_flux_coverage_rows="
                  << rawFluxCoverageRowsMax << std::endl;
        assert(false);
    }

    const bool bootstrap = (std::getenv("WEEK79_BOOTSTRAP_HARDCORPUS_REF") != nullptr);
    if (bootstrap) {
        writeWeek79RealHardCorpusTraceCsv(refPath, got);
        std::cout << "[WEEK79-HARD][HARDREF] BOOTSTRAP: wrote " << refPath << std::endl;
        return;
    }

    const std::vector<std::string> ref = readTextRowsNoHeader(refPath);
    if (ref.empty()) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: missing/empty reference " << refPath << std::endl;
        assert(false);
    }
    if (ref.size() != got.size()) {
        std::cerr << "[WEEK79-HARD][HARDREF] FAIL: row count mismatch got=" << got.size()
                  << " ref=" << ref.size() << std::endl;
        assert(false);
    }
    for (size_t i = 0; i < got.size(); ++i) {
        if (got[i] != ref[i]) {
            std::cerr << "[WEEK79-HARD][HARDREF] FAIL: mismatch row=" << i
                      << " got='" << got[i] << "'"
                      << " ref='" << ref[i] << "'" << std::endl;
            assert(false);
        }
    ...
    std::cout << "[WEEK79-HARD][HARDREF] PASS: real hard corpus trace matches reference"
              << " hard_rows=" << hardCorpusRowsMax
              << " timing_regressions=" << timingRegressionsMax
              << " fallback_rows=" << fallbackRowsMax
