...
function Run-Binary {
    param(
        ...
        [bool]$NeedIecCopyFileE2E,
        [bool]$NeedIecCopyDiskE2E,
        [hashtable]$ExtraEnv
    )
    ...
    try {
        ...
        if ($NeedIecCopyFileE2E -and ($text -notmatch "\[IEC COPY E2E\] PASS: copy_8_to_9_file_e2e")) { return ,@($false, $output, $exitCode) }
        if ($NeedIecCopyDiskE2E -and ($text -notmatch "\[IEC COPY E2E\] PASS: copy_8_to_9_disk_e2e")) { return ,@($false, $output, $exitCode) }
        if ($NeedIecCopyDiskE2E) {
            $manifestPath = Join-Path -Path $repo -ChildPath "copy_8_to_9_disk_e2e_manifest.csv"
            if (-not (Test-Path -LiteralPath $manifestPath)) { return ,@($false, $output, $exitCode) }
            $manifestText = Get-Content -LiteralPath $manifestPath -Raw
            if ($manifestText -notmatch "match" -or $manifestText -notmatch ",1\s*$") { return ,@($false, $output, $exitCode) }
        }
...
try {
    ...
    $results += Invoke-Step -Name "run-full" -Action {
        $r = Run-Binary -ExePath "$repo\c64_11_full_signoff.exe" -ManifestPath $Manifest -NeedWeek45:$true -NeedWeek46:$true -NeedWeek47:$true -NeedWeek48:$true -NeedWeek49:$true -NeedWeek50:$true -NeedWeek51:$true -NeedWeek52:$true -NeedWeek53:$true -NeedWeek54:$true -NeedWeek55:$true -NeedWeek56:$true -NeedWeek57:$true -NeedWeek58:$true -NeedWeek59:$true -NeedWeek60:$true -NeedWeek61:$true -NeedWeek62:$true -NeedWeek63:$true -NeedWeek64:$true -NeedWeek65:$true -NeedWeek66:$true -NeedWeek67:$true -NeedWeek68:$true -NeedWeek69:$true -NeedWeek70:$true -NeedWeek71:$true -NeedWeek72:$true -NeedWeek73:$true -NeedWeek74:$true -NeedWeek75:$true -NeedWeek76:$true -NeedWeek77:$true -NeedWeek78:$true -NeedWeek79:$true -NeedWeek80:$true -NeedWeek81:$true -NeedWeek12:$true -NeedExternal:$false -NeedNoFallback:$false -NeedIecMultiUnit:$false -NeedIecCopyFileE2E:$false -NeedIecCopyDiskE2E:$false -ExtraEnv $null
        $script:__runFull = $r
        if (-not $r[0]) { foreach ($line in $r[1]) { $line }; exit $r[2] }
    } -Assert { param($o, $e) $e -eq 0 }
